...
int main(int argc, char** argv)
{
    ...
    load_file(responseList, argv[1]); // whole text file given
...
// takes tuple <keyword, reponseword, happiness> file 
// for each line in file
//      find out AI's response to line
//      place it in corresponding spot in vector
void load_file(vector <Response*>& respList, char* fileName)
{
    ...
    string word;
    while(inFile >> word) {
        // inFile is an entire line from the text file in each iteration
        // inFile >> x moves file pointer to next word in the line and
        //             puts that word in to x
        ...
        // -1 -> sad
        int happy;
        inFile >> word; // it's ok to use word bc keyword saved a copy
        // Convert string to an int using stringstream
        stringstream tmp(word); // stringstream must be a class
        tmp >> happy;
        ...
        if (happy == 0) {
            ...
            respList.push_back(r); // adds r to end of respList vector
        } else if (happy > 0) {
               ...
               char* outFileName)
{
    ...
    ifstream msgFile(msgFileName); // ifstream for reading files
    ofstream outFile(outFileName); // ofstream for writing
...
void respondToInput(vector <Response*> respList, string input, 
                    ofstream& output)
{   // input is a line from message file
    // output is the result file
    // respList is the vector of 
    bool found = false;
...
// $ ./ai test2/input.txt test2/message.txt test2/liamsOut.txt