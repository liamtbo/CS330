...
bool Response::checkAndRespond(const string& inWord, ostream& toWhere)
{
    ...
    // does keyword exist in the line?

    // TODO: this is not returning true when it should be
    if (inWord.find(this->keyword.upper()) != std::string::npos) {
...
void Response::respond(ostream& toWhere)
{
    ...
    toWhere << "I am neither angry nor happy: " + this->response + "\n";
}
...
void AngryResponse::respond(ostream& toWhere)
{
  ...
  // is inserted to the toWhere output stream, along with a newline at the end
    toWhere << "I am angry: " + this->response + "\n";
}
...
// is inserted to the toWhere output stream, along with a newline at the end
    toWhere << "I am happy: " + this->response + "\n";
}
