...
int main(int argc, char** argv) 
{
    ...
    // load the adjacency matrix from file (vertex * neighbor)
    int** int_array = NULL; // 2D-matrix populated with vertex adjacencies (0 or 1)
    uint32_t row = 0; // number of rows
    uint32_t col = 0; // number of cols
    uint32_t nnz = 0; // number of non-zeros
