                 ...
                 uint32_t* row, uint32_t* col, uint32_t* nnz)
{
    ...
    while(fscanf(fp, "%d", &tmp) == 1) {
        ...
        if(tmp > 0) {
            cnt_nz++; // number of non-zero elements
        }
    }
    fclose(fp);
    ...
    // Read the first line (not needed anymore) & move file pointer down
    fscanf(fp, "%d", &tmp);
    fscanf(fp, "%d", &tmp);
    fscanf(fp, "%d", &tmp);
    ...
    while (fscanf(fp, "%d", &tmp) == 1) {
        // cnt / c is integer division, so only keep quotient
