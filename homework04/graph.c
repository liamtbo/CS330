...
int remove_node(adj_node_t** list)
{
    ...
    if(list != NULL) {
        ...
        if(head != NULL) {
            ...
            // capture value
            vid = head->vid;
            // reassign head to the next in line
            head = head->next;
         ...
         int* color, int* distance, int* parent)
{
    ...
    adj_node_t **q = NULL;
    init_adj_list(&q, rows); // one row because we're using it as a queue
    // add the source node as the first elem. in queue
    ...
    while (q[0] != NULL) {
        // since we only populate row0 for the queue, list is a ** to row 0
        int used_node = remove_node(q);
        adj_node_t *used_nodes_adj = list[used_node];
        // loop through used nodes adjacent verticies
        while (used_nodes_adj != NULL) {
            if (color[used_nodes_adj->vid] == 0) {
                color[used_nodes_adj->vid] = 1;
                parent[used_nodes_adj->vid] = used_node;
                distance[used_nodes_adj->vid] = distance[used_node] + 1;
                add_node(q, 0, create_node(used_nodes_adj->vid));
            }
            used_nodes_adj = used_nodes_adj->next;
        }
        color[used_node] = 2;
    }
